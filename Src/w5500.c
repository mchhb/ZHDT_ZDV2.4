...
void sysinit( uint8_t * tx_size, uint8_t * rx_size  )
{
  ...
  {
          IINCHIP_WRITE( (Sn_TXMEM_SIZE(i)), tx_size[i]);		// MCU¼ÇÂ¼Ã¿¸öSocket·¢ËÍ»º´æµÄ´óĞ¡µ½Êı×étx_size[]¶ÔÓ¦µÄÔªËØÖµ
          IINCHIP_WRITE( (Sn_RXMEM_SIZE(i)), rx_size[i]);		// MCU¼ÇÂ¼Ã¿¸öSocket½ÓÊÕ»º´æµÄ´óĞ¡µ½Êı×érx_size[]¶ÔÓ¦µÄÔªËØÖµ
...
/**
@brief  W5500ĞÄÌø¼ì²â³ÌĞò£¬ÉèÖÃSocketÔÚÏßÊ±¼ä¼Ä´æÆ÷Sn_KPALVTR£¬µ¥Î»Îª5s
*/
...
{
   return IINCHIP_READ(Sn_SR(s)); // MCU¶ÁSn_SR¶ÔÓ¦µØÖ·ÀïÃæµÄÊıÖµ²¢·µ»Ø
}
...
*/
uint16_t getSn_RX_RSR(uint8_t s)		// »ñÈ¡Sn_RX_RSR¿ÕÏĞ½ÓÊÕ»º´æ¼Ä´æÆ÷µÄÖµ²¢·µ»Ø,Sn_RX_RSRÖĞ±£´æµÄÊÇ½ÓÊÕ»º´æÖĞÒÑ½ÓÊÕºÍ±£´æµÄÊı¾İ´óĞ¡
{
  uint16_t val=0,val1=0;
  do
  {
    ...
    if(val1 != 0)																				// Èç¹ûSn_RX_RSRµÄÖµ²»Îª0£¬ËµÃ÷½ÓÊÕ»º´æÖĞÊÕµ½ÁËÊı¾İ
    {
...
*/
void send_data_processing(uint8_t s, uint8_t *data, uint16_t len)		// MCU°ÑÊı¾İ·¢ËÍ¸øW5500µÄ¹ı³Ì£¬W5500½«Êı¾İĞ´Èëbuf£¬²¢¸üĞÂÊı¾İµÄĞ´Ö¸ÕëµÄµØÖ·
{
  uint16_t ptr =0;
  uint32_t addrbsb =0;
...
// Sn_RX_WRÊÇ·¢ËÍĞ´Ö¸Õë¼Ä´æÆ÷£¬ÓÃÀ´±£´æ·¢ËÍ»º´æÖĞ½«Òª´«ÊäÊı¾İµÄÊ×µØÖ· 
  ptr = IINCHIP_READ( Sn_TX_WR0(s) );
  ptr = ((ptr & 0x00ff) << 8) + IINCHIP_READ(Sn_TX_WR1(s));			// MCU¶ÁÈ¡Sn_RX_WR¼Ä´æÆ÷µÄÖµ

  addrbsb = (uint32_t)(ptr<<8) + (s<<5) + 0x10;										// ½«·¢ËÍ»º´æÖĞ½«Òª´«ÊäÊı¾İµÄÊ×µØÖ·×ª³É32Î»
  wiz_write_buf(addrbsb, data, len);														// W5500´Ó¸ÃÊ×µØÖ·¿ªÊ¼Ğ´ÈëÊı¾İ£¬Êı¾İ³¤¶ÈÎªlen
  
  ptr += len;																										// Ê×µØÖ·µÄÖµ+len±äÎªÊı¾İĞÂµÄÊ×µØÖ·
  IINCHIP_WRITE( Sn_TX_WR0(s) ,(uint8_t)((ptr & 0xff00) >> 8));
  IINCHIP_WRITE( Sn_TX_WR1(s),(uint8_t)(ptr & 0x00ff));						// ½«ĞÂµÄÊ×µØÖ·±£´æÔÚSn_RX_WRÖĞ
}
...
// Sn_RX_RDÊÇ·¢ËÍ¶ÁÖ¸Õë¼Ä´æÆ÷
  ...
  addrbsb = (uint32_t)(ptr<<8) + (s<<5) + 0x18;					// ½«·¢ËÍ»º´æÖĞ½«Òª´«ÊäÊı¾İµÄÊ×µØÖ·×ª³É32Î»
  wiz_read_buf(addrbsb, data, len);										// W5500´Ó¸ÃÊ×µØÖ·¿ªÊ¼¶ÁÈ¡Êı¾İ£¬Êı¾İ³¤¶ÈÎªlen
  ptr += len;																					// Ê×µØÖ·µÄÖµ+len±äÎªÊı¾İĞÂµÄÊ×µØÖ·

  IINCHIP_WRITE( Sn_RX_RD0(s), (uint8_t)((ptr & 0xff00) >> 8));		
  IINCHIP_WRITE( Sn_RX_RD1(s), (uint8_t)(ptr & 0x00ff));		// ½«ĞÂµÄÊ×µØÖ·±£´æÔÚSn_RX_RDÖĞ
}
...
{	
			switch(getSn_SR(0))										// »ñÈ¡socket0µÄ×´Ì¬
			{
				...
				case SOCK_ESTABLISHED:							// Socket´¦ÓÚÁ¬½Ó½¨Á¢×´Ì¬
						...
						// Êı¾İ»Ø»·²âÊÔ³ÌĞò£ºÊı¾İ´ÓÉÏÎ»»ú¿Í»§¶Ë·¢¸øW5500£¬W5500½ÓÊÕµ½Êı¾İºóÔÙ»Ø¸ø¿Í»§¶Ë
						revbuffer_len=getSn_RX_RSR(0);						// ¶ÁÈ¡W5500¿ÕÏĞ½ÓÊÕ»º´æ¼Ä´æÆ÷µÄÖµ²¢¸³¸ølen£¬Sn_RX_RSR±íÊ¾½ÓÊÕ»º´æÖĞÒÑ½ÓÊÕºÍ±£´æµÄÊı¾İ´óĞ¡
						if(revbuffer_len>0)
						{
...
//							send(0,USART_RX_BUF,len);			// W5500Ïò¿Í»§¶Ë·¢ËÍÊı¾İ
...
//						}
				...
				case SOCK_CLOSED:										// Socket´¦ÓÚ¹Ø±Õ×´Ì¬
						socket_flag = 1;
						socket(0,Sn_MR_TCP,local_port,Sn_MR_ND);		// ´ò¿ªSocket0£¬²¢ÅäÖÃÎªTCPÎŞÑÓÊ±Ä£Ê½£¬´ò¿ªÒ»¸ö±¾µØ¶Ë¿Ú
				break;
				default:
					close(0);
				break;
